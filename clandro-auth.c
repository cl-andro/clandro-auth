...
unsigned char *clandro_passwd_hash(const char *password) {
    const unsigned char *salt = (const unsigned char *) "Clandro!";
...
bool clandro_change_passwd(const char *new_password) {
    FILE *clandro_auth_file;
    bool is_password_changed = false;

    unsigned char *hashed_password = clandro_passwd_hash(new_password);
    if (!hashed_password) {
        return false;
    }

    if ((clandro_auth_file = fopen(AUTH_HASH_FILE_PATH, "w")) != NULL) {
        int n = fwrite(hashed_password, sizeof(unsigned char), SHA_DIGEST_LENGTH, clandro_auth_file);
        fflush(clandro_auth_file);
        fclose(clandro_auth_file);
...
bool clandro_auth(const char *user, const char *password) {
    ...
    if ((hashed_password = clandro_passwd_hash(password)) == NULL) {
        free(auth_info);
        return false;
    }

    if ((clandro_auth_file = fopen(AUTH_HASH_FILE_PATH, "rb")) != NULL) {
        int n = fread(auth_info, sizeof(unsigned char), SHA_DIGEST_LENGTH, clandro_auth_file);
        fclose(clandro_auth_file);
