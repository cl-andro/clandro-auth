//
// Password authentication utilities for Clandro
...
char *get_shell() {
    ...
    if (lstat(CLANDRO_HOME "/.clandro/shell", &st) == 0) {
        if (S_ISLNK(st.st_mode)) {
            // If ~/.clandro/shell points to executable.
            char link_target[PATH_MAX];
            ssize_t len = readlink(CLANDRO_HOME "/.clandro/shell", link_target, sizeof(link_target) - 1);
            ...
            // If ~/.clandro/shell is regular executable file.
            // Some Clandro users may prefer to launch a custom program
            // instead of shell.
            if (access(CLANDRO_HOME "/.clandro/shell", R_OK | X_OK) == 0) {
                shell = CLANDRO_HOME "/.clandro/shell";
            }
        } else {
            // If ~/.clandro/shell has unexpected file type.
            fprintf(stderr, "Warning: ~/.clandro/shell is not a regular file or symbolic link.\n");
        }
    } else {
...
// Setup the clandro-exec hook if possible.
void prepare_clandro_exec() {
    pid_t ch_pid, pid;
    int status;

    setenv("LD_PRELOAD", CLANDRO_PREFIX "/lib/libclandro-exec.so", 1);

    ch_pid = fork();

    if (ch_pid == 0) {
        execl(CLANDRO_PREFIX "/bin/sh", "sh", "-c", "true", NULL);
    } else if (ch_pid > 0) {
        pid = wait(&status);
        if (WIFEXITED(status)) {
...
void init_login() {
    ...
    if (show_motd && (motd = fopen(CLANDRO_PREFIX "/etc/motd", "rb")) != NULL) {
        char *buf[256];
        int bytes_read;
...
int main(int argc, char **argv) {
    ...
    if (access(CLANDRO_HOME "/.hushlogin", F_OK) == 0 || getenv("CLANDRO_HUSHLOGIN") != NULL) {
        show_motd = false;
        unsetenv("CLANDRO_HUSHLOGIN");
    }

    for (int attempt=0; attempt<3; attempt++) {
        char *password = getpass("Clandro password: ");
        ...
        if (clandro_auth("clandro", password)) {
            init_login();
        } else {
